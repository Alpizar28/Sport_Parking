                ...
                {Array.from({ length: days }).map((_, i) => {
                    ...
                    return (
                        <button
                            ...
                            disabled={isPast}
                            className={`
                                h-10 w-10 flex items-center justify-center rounded-xl transition-all font-medium
                                ${isSelected
                                    ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-500/30 scale-105'
                                    : isPast
                                        ? 'text-gray-300 bg-transparent cursor-not-allowed'
                                        : 'text-gray-700 hover:bg-emerald-50 hover:text-emerald-700 bg-white/50'
                                }
                                ${isToday && !isSelected ? 'ring-2 ring-emerald-500/30 text-emerald-600 font-bold' : ''}
                            `}
                        >
                            {day}
                        </button>
